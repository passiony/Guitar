...
public class SongDataEditor : Editor
{
    ...
    protected void RedrawProgressViewTexture()
    {
        ...
        for (int i = 0; i < SongPlayer.Song.Notes.Count; ++i)
        {
            ...
            int y = (int)Mathf.Round(((SongPlayer.Song.Notes[i].Time + SongPlayer.Song.AudioStartBeatOffset - 1) /
                                      totalBeats) * height);
    ...
    protected void OnKeyDown(Event e)
    {
        switch (e.keyCode)
        {
            case KeyCode.UpArrow:
                ...
                GuitarObject.GetComponent<AudioSource>().time +=
                    MyMath.BeatsToSeconds(1f, SongPlayer.Song.BeatsPerMinute);
                e.Use();
                break;
    ...
    protected void DrawInspector()
    {
        ...
        GuitarObject.GetComponent<AudioSource>().pitch =
            GUILayout.HorizontalSlider(GuitarObject.GetComponent<AudioSource>().pitch, 0, 1);
        EditorGUILayout.EndHorizontal();
    ...
    {
        return new Rect(SongViewRect.width - SongViewProgressBarWidth, SongViewRect.yMin, SongViewProgressBarWidth,
            SongViewRect.height);
    }
    ...
    protected void DrawProgressViewTimeMarker()
    {
        ...
        {
            previewProgress = GuitarObject.GetComponent<AudioSource>().time /
                              GuitarObject.GetComponent<AudioSource>().clip.length;
        }

        float previewProgressTop = rect.yMin + rect.height * (1 - previewProgress);
        DrawLine(new Vector2(rect.xMin, previewProgressTop), new Vector2(rect.xMax + rect.width, previewProgressTop),
            Color.red, 2);
    }
    ...
    {
        DrawVerticalLine(new Vector2(rect.xMin + rect.width / 2, rect.yMin),
            new Vector2(rect.xMin + rect.width / 2, rect.yMax), color, (int)rect.width, clip);
    }
